...
export default function Carretel({ cor = "#3b82f6", porcentagem = 100, tamanho = 128, className = "" }) {
    ...
    // Detectar se a cor é muito escura para adicionar suporte de contorno "wireframe"
    const ehEscuro = useMemo(() => {
        const rgb = parseInt(cor.replace('#', ''), 16);
        const r = (rgb >> 16) & 0xff;
        const g = (rgb >> 8) & 0xff;
        const b = (rgb >> 0) & 0xff;
        const luma = 0.2126 * r + 0.7152 * g + 0.0722 * b; // Pesos SMPTE C, Rec. 709
        return luma < 40; // Limiar para "muito escuro"
    }, [cor]);
