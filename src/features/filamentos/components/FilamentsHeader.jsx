                        ...
                        <div>
                            ...
                            <div className="flex items-baseline gap-1">
                                <span className="text-3xl font-black text-white tracking-tighter">
                                    {(stats?.valorTotal || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}
                                </span>
                            </div>
                        </div>
                    ...
                    <div className="flex items-start justify-between relative z-10">
                        <div>
                            ...
                            {lowStockCount > 0 ? (
                                <div className="flex items-center gap-2">
                                    <span className="text-xl font-bold text-rose-400 uppercase tracking-tight">
                                        {lowStockCount} Carretéis Baixos
                                    </span>
                                </div>
                            ) : (
                                <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] text-zinc-600 block mt-1 leading-tight">
                                {lowStockCount > 0 ? "Considere reposição em breve." : "Nenhuma ação necessária no momento."}
                            </span>
                        </div>
                        <div className={`p-3 rounded-2xl border ${lowStockCount > 0 ? 'bg-rose-500/10 border-rose-500/20 text-rose-400 animate-pulse' : 'bg-zinc-800/50 border-white/5 text-zinc-600'}`}>
                ...
                <div className="flex items-center gap-3">
                    <div className="hidden md:flex p-1 bg-zinc-900/80 border border-white/5 rounded-xl">
                        <button
                            ...
                            className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-zinc-800 text-zinc-100 shadow-sm' : 'text-zinc-600 hover:text-zinc-300'}`}
                        ...
                        <button
                            ...
                            className={`p-2 rounded-lg transition-all ${viewMode === 'table' ? 'bg-zinc-800 text-zinc-100 shadow-sm' : 'text-zinc-600 hover:text-zinc-300'}`}
