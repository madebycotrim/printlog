    ...
    <div className="w-80 bg-[#0c0c0e]/80 backdrop-blur-xl border border-white/10 rounded-[2rem] p-6 shadow-2xl">
        <div className="flex justify-between items-start mb-6">
            <div className="space-y-1">
                <Badge label="Recuperação" color="amber" icon={Settings} />
                <h4 className="text-white font-bold text-lg mt-2 uppercase">Acesso à Conta</h4>
            </div>
            <Cpu className="text-zinc-700" size={20} />
        </div>
        <div className="space-y-3">
            {[
                { label: 'Segurança', status: 'Ativa', icon: ShieldCheck },
                { label: 'Sincronização', status: 'Online', icon: RefreshCcw }
            ].map((item, i) => (
...
export default function ForgotPasswordPage() {
    ...
    const handleSendCode = async (e) => {
        ...
        try {
            ...
            setError(err.errors?.[0]?.longMessage || "E-mail não encontrado.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            ...
            setError(err.errors?.[0]?.longMessage || "Código inválido ou senha muito curta.");
        } finally {
            setIsLoading(false);
        }
    };
                ...
                <div className="w-full max-w-md space-y-10">
                    <div className="space-y-4 text-center sm:text-left">
                        <div className="flex items-center gap-3 justify-center sm:justify-start">
                            ...
                            <span className="text-xl font-bold text-white uppercase tracking-tight">PrintLog <span className="text-sky-500 text-[10px] ml-1 uppercase">Recovery</span></span>
                        </div>
                        <div className="space-y-2">
                            <h2 className="text-4xl sm:text-5xl font-black tracking-tighter leading-[0.95] text-white uppercase">
                                {step === 1 ? "RECUPERAR" : "CRIAR NOVA"} <br />
                            ...
                            <p className="text-zinc-500 text-sm font-medium">
                                {step === 1
                                    ? "Siga os passos abaixo para recuperar sua conta e voltar ao workshop."
                    ...
                    <form onSubmit={step === 1 ? handleSendCode : handleResetPassword} className="space-y-6">
                        {step === 1 ? (
                            <div className="space-y-5">
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-zinc-500 ml-1">Seu e-mail cadastrado</label>
                            ...
                            <div className="space-y-6">
                                <div className="space-y-4 text-center bg-sky-500/5 border border-sky-500/20 p-8 rounded-[2rem]">
                                    <label className="text-xs font-bold uppercase tracking-widest text-sky-500 block">Código enviado</label>
                                    <input
                                ...
                                <div className="space-y-2 group">
                                    <label className="text-xs font-bold text-zinc-500 ml-1">Escolha uma nova senha</label>
                                    <div className="relative">
                    ...
                    <div className="bg-[#0c0c0e]/90 backdrop-blur-xl border border-sky-500/20 rounded-[2rem] p-6 shadow-2xl mt-6 mx-auto w-72">
                        <div className="flex items-center gap-4 mb-3">
                            ...
                            <div className="text-left">
                                <span className="text-[10px] font-bold uppercase text-emerald-500 block">Tudo Seguro</span>
                                <span className="text-[12px] font-bold text-white uppercase">Dados Protegidos</span>
                            </div>
                        </div>
