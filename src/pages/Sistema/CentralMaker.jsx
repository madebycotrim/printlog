...
const SimpleCard = ({ title, subtitle, icon: Icon, color = "sky", children, badge }) => {
    ...
    // Extrair apenas a cor do texto para o Ã­cone (hack limpo)
        ...
        <div className="bg-zinc-950/40 border border-zinc-800/50 rounded-2xl p-6 hover-lift transition-all duration-300 group">
            ...
            {badge && (
                <div className="mt-4 flex justify-end">
                    <span className="text-[9px] font-black px-2 py-1 rounded-md bg-zinc-950/40 border border-zinc-800 text-zinc-600 uppercase tracking-widest opacity-60 group-hover:opacity-100 transition-opacity">
                        {badge}
                    </span>
                </div>
            )}
...
export default function CentralMaker() {
    const { width: sidebarWidth } = useSidebarStore();
        ...
        <div className="flex h-screen w-full bg-zinc-950 text-zinc-200 font-sans antialiased overflow-hidden">
            ...
            <main className="flex-1 flex flex-col relative overflow-y-auto custom-scrollbar" style={{ marginLeft: `${sidebarWidth}px` }}>
                ...
                <div className="p-8 xl:p-12 relative z-10 max-w-[1600px] mx-auto w-full">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 animate-fade-in-up">
                        <div>
                            ...
                            <p className="text-sm text-zinc-500 max-w-lg leading-relaxed">
                                Base de conhecimento para otimizar sua produÃ§Ã£o e resolver problemas tÃ©cnicos comuns.
                            </p>
                        </div>
                        ...
                        <div className="relative group w-full md:w-96">
                            ...
                            <input
                                className="w-full bg-zinc-950/50 border border-zinc-800/80 rounded-xl py-3.5 pl-12 pr-4 text-xs font-bold text-zinc-300 outline-none focus:border-sky-500/50 focus:bg-zinc-950/40 transition-all placeholder:text-zinc-700 uppercase tracking-wider shadow-inner font-mono"
                    ...
                    {/* Filtros em Tabs - Visual de 'PÃ­lulas' do Dashboard */}
                    <div className="flex items-center gap-2 mb-8 overflow-x-auto pb-2 custom-scrollbar animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                        {[
                            ...
                            { id: 'critico', label: 'CrÃ­ticos' },
                            { id: 'lucro', label: 'Financeiro' },
                            { id: 'setup', label: 'TÃ©cnico' },
                        ].map((filter) => (
                            <button
                                key={filter.id}
                                ...
                                className={`px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] transition-all border ${selectedFilter === filter.id
                                    ? 'bg-zinc-900/50 text-zinc-200 border-zinc-800/50 shadow-lg'
                                    ...
                                    {category.topics.map(topic => (
                                        ...
                                        >
                                            <div className="flex items-center gap-2.5 overflow-hidden">
                                                <div className="w-1 h-1 rounded-full bg-zinc-900/50 group-hover/item:bg-sky-500 transition-colors shrink-0" />
