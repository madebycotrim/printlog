...
import { Activity, LayoutGrid, RotateCcw, EyeOff, Eye, Plus, Check, Box, Printer, Package, Calculator, Layers, Search, Command } from 'lucide-react';
...
// Importar modais para aÃ§Ãµes rÃ¡pidas
...
export default function Dashboard() {
    ...
    const resetLayout = useCallback(() => {
        originalResetLayout();
        addToast('Layout restaurado para o padrÃ£o.', 'success');
    ...
    // Estados para modais de aÃ§Ãµes rÃ¡pidas
    ...
    // Handlers de AÃ§Ãµes Inteligentes
    ...
    return (
        <div className="flex h-screen w-full bg-zinc-950 text-zinc-200 font-sans antialiased overflow-hidden">
            ...
            >
                ...
                <div className="relative z-10 p-8 xl:p-12 max-w-[1600px] mx-auto w-full">
                            ...
                            <div className="flex items-center gap-3 w-full md:w-auto">
                                ...
                                <div className="hidden md:block relative group mr-2">
                                    <div
                                        ...
                                        className="
                                            flex items-center gap-3 w-[240px] px-3 py-2.5 
                                            bg-zinc-900/50 border border-zinc-800/50 rounded-xl 
                                            text-zinc-500 cursor-pointer transition-all hover:bg-zinc-800/50 hover:border-zinc-700 hover:text-zinc-400
                                            active:scale-95
                                        "
                                    >
                                        <Search size={14} />
                                        <span className="text-[11px] font-bold uppercase tracking-widest flex-1">Buscar...</span>
                                ...
                                {/* GRUPO DE AÃ‡Ã•ES RÃPIDAS (Novo Projeto, Filamento, Impressora) */}
                                <div className="flex items-center gap-2 bg-zinc-900/50 p-1.5 rounded-2xl border border-zinc-800/50 shadow-sm">

                                    {/* NOVO PROJETO (Calculadora - Sky) */}
                                    <button
                                        ...
                                        className="
                                            ...
                                            shadow-lg shadow-sky-500/20 transition-all active:scale-95
                                        "
                                        title="Criar novo orÃ§amento ou projeto"
                                    >
                                        <Calculator size={18} strokeWidth={3} />
                                        <span className="hidden sm:inline uppercase text-[10px] tracking-widest">Novo Projeto</span>
                                    </button>

                                    <div className="w-px h-6 bg-zinc-800 mx-1" />

                                    {/* ADD FILAMENTO (Package - Rose) */}
                                    <button
                                        onClick={() => setFilamentModalOpen(true)}
                                        className="
                                            p-2.5 rounded-xl text-zinc-400 hover:text-rose-400 hover:bg-rose-500/10 
                                            transition-all active:scale-95
                                        "
                                        title="Adicionar novo filamento ao estoque"
                                    >
                                        <Package size={20} strokeWidth={2} />
                                    </button>

                                    {/* ADD INSUMO (Layers - Orange) */}
                                    <button
                                        onClick={() => setSupplyModalOpen(true)}
                                        className="
                                            p-2.5 rounded-xl text-zinc-400 hover:text-orange-400 hover:bg-orange-500/10 
                                            transition-all active:scale-95
                                        "
                                        title="Adicionar novo insumo ao estoque"
                                    >
                                        <Layers size={20} strokeWidth={2} />
                                    </button>

                                    {/* ADD IMPRESSORA (Printer - Emerald) */}
                                    <button
                                        onClick={() => setPrinterModalOpen(true)}
                                        className="
                                            p-2.5 rounded-xl text-zinc-400 hover:text-emerald-400 hover:bg-emerald-500/10 
                                            transition-all active:scale-95
                                        "
                                ...
                                <button
                                    ...
                                    className={`group flex items-center justify-center p-3 rounded-xl border transition-all duration-300 spring-bounce ${editMode
                                        ? 'bg-sky-500 text-white border-sky-500 shadow-lg shadow-sky-500/30 hover:shadow-sky-500/50'
                                        : 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 hover:border-zinc-700'
                                        } `}
                                    title={editMode ? "Salvar e sair do modo de ediÃ§Ã£o" : "Entrar no modo de ediÃ§Ã£o"}
                                    aria-label={editMode ? "Salvar Layout" : "Reorganizar Dashboard"}
                                >
                                    {editMode ? <Check size={20} className="group-hover:scale-110 transition-transform" /> : <LayoutGrid size={20} className="group-hover:rotate-90 transition-transform" />}
                                </button>

                                {editMode && (
                                    <button
                                        onClick={resetLayout}
                                        className="group flex items-center justify-center p-3 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-800 hover:border-zinc-700 transition-all duration-300 spring-bounce"
                                        aria-label="Resetar para Layout PadrÃ£o"
                                    >
                    ...
                    {layout.length === 0 && (
                        <div className="flex flex-col items-center justify-center p-12 text-center rounded-3xl border-2 border-dashed border-zinc-800 bg-zinc-900/30 min-h-[400px]">
                            ...
                            <h3 className="text-xl font-bold text-white mb-2">Seu Dashboard estÃ¡ vazio</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 pb-32 auto-rows-[minmax(200px,auto)]" style={{ gridAutoFlow: 'dense' }}>
                                            ...
                                            {editMode && (
                                                <div className="absolute top-4 right-4 z-50 flex items-center gap-1 bg-zinc-900/90 backdrop-blur-md rounded-lg p-1 border border-zinc-800 shadow-xl scale-100 animate-in fade-in zoom-in duration-200 pointer-events-auto">
                                                    ...
                                                    <div className="flex items-center gap-1 pr-2 border-r border-white/10 mr-1">
                                                        <button
                                                            onClick={() => setWidgetSize(widgetId, 1, 1)}
                                                        ...
                                                        <button
                                                            onClick={() => setWidgetSize(widgetId, 2, 1)}
                                                            className="p-1 hover:bg-white/10 rounded text-zinc-400 hover:text-white"
                                                            title="2x1 (Largo)"
                                                        >
                                                        ...
                                                        <button
                                                            onClick={() => setWidgetSize(widgetId, 1, 2)}
                                                            className="p-1 hover:bg-white/10 rounded text-zinc-400 hover:text-white"
                                                            title="1x2 (Alto)"
                                                        >
                                                        ...
                                                        <button
                                                            onClick={() => setWidgetSize(widgetId, 2, 2)}
                                                            className="p-1 hover:bg-white/10 rounded text-zinc-400 hover:text-white"
                                                            title="2x2 (Grande)"
                                                        >
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-1.5 rounded-md hover:bg-rose-500/20 text-zinc-400 hover:text-rose-400 transition-all"
                ...
                >
                    <div className="bg-zinc-950/90 backdrop-blur-xl border border-zinc-800/60 rounded-2xl px-6 py-4 shadow-2xl shadow-black/50 min-w-[600px] max-w-[90vw]">
                        ...
                        {hidden.length === 0 ? (
                            <div className="flex-1 text-center py-2 text-zinc-600 text-sm font-medium italic">
                                Todos os widgets estÃ£o ativos
                            </div>
                        ) : (
                ...
                {/* MODAIS (Renderizados no nÃ­vel na pagina) */}
                <ModalFilamento
